...
object 热点问题 extends LoadHadoop{
...
object 热点标签 extends LoadHadoop{
  def main(args: Array[String]): Unit = {
    val sql1 = """
    select t.tags,
           count(*) as tags_num
    ...
    import org.apache.spark.sql.functions._
    data.select(explode(split(column("answerer_tags"), " ")).as("tags"))
      .filter("length(tags)>1")
      .groupBy("tags")
...
object 高赞用户 extends LoadHadoop{
  def main(args: Array[String]): Unit = {
    val sql1 = """
    select answerer_tags,
           sum(star) as star_sum,
           collect_list(star) as star_list
    from qa
    where length(answerer_tags) >1
...
object 高频词 extends LoadHadoop{
  def main(args: Array[String]): Unit = {
    data.select("content")
      .flatMap(row=>BaseAnalysis.parse(row.getString(0))
        .getTerms
        .asScala
      ...
      .toDF("words")
      .createTempView("toks")
    ss.sql("select * from toks").show(100)
...
object 情感分析 extends LoadHadoop{
  def main(args: Array[String]): Unit = {
    ...
    val posDict = ss.sparkContext.textFile("input\\dict\\正面词.dict").filter(_.length>1).map(x=>Row(x,1))
    val negDict = ss.sparkContext.textFile("input\\dict\\负面词.dict").filter(_.length>1).map(x=>Row(x,-1))
    ...
    data.select("qid", "title","content")
      ...
      .toDF("qid","title", "word")
      .createTempView("toks")
    ss.sql("select * from toks").show(100,false)

    val sql1 = """
    select t.qid,
           first(t.title) as title,
           sum(t.score) as score_sum,
           collect_list(t.score) as score_list
    from (
...
object 自定义函数 extends LoadHadoop{
  def main(args: Array[String]): Unit = {
    // 注册agg函数
...
object 添加分类回归标记 extends LoadHadoop{
  def main(args: Array[String]): Unit = {
    val dictScheme = StructType(Array(StructField("word",StringType),StructField("score",IntegerType)))
...
object 线性回归 extends LoadHadoop{
  def main(args: Array[String]): Unit = {
    ...
    test_result_nb.select("reg_score","prediction").show(100,false)
